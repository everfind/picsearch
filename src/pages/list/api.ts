...
export function useGetPhotos({ keyword }: { keyword: string }): [Picture[], () => void] {
    ...
    const getMore = useCallback(async () => {
        const pexelsResult: Promise<Picture[]> = Pexels.search({ query: keyword, page, per_page: perPage }).then((photos) => {
            return photos.photos.map((tt) => ({ id: tt.id, pageUrl: tt.url, imageUrl: tt.src.large, width: tt.width, height: tt.height }));
        });
        const pixabayResult: Promise<Picture[]> = Pixabay.search({ q: keyword, page, per_page: perPage }).then((photos) => {
